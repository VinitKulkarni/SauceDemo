...
public class AddPersonInfoPageTest extends SauceDemoBaseClass {
	...
	public void preSteps() throws IOException {
		...
		if(flag == true) {
			//first time product is added to cart thats why below lines
			productPageObj.clickOnAddToCart();
			productPageObj.clickOnViewCart();
		}else {
			//already product added to cart. directly click on cart view and checkout
			productPageObj.clickOnViewCart();
		}
	...
	@Test(priority = 2)
	public void validate_by_adding_all_fields() throws EncryptedDocumentException, IOException {
		...
		assertEquals(driver.getCurrentUrl(), UtilityClass.readDataFromExcelFile(5, 0));
		flag = false;
	}
	
	@Test(priority = 3)
	public void validate_by_not_adding_all_fields() throws EncryptedDocumentException, IOException {
		addPersonInfoPageObj.setFirstName("");
		addPersonInfoPageObj.setLastName("");
		addPersonInfoPageObj.setZipCode("");
		addPersonInfoPageObj.clickOnContinueButton();
		
		assertEquals(addPersonInfoPageObj.getEmptyFieldsErrorMsg(), UtilityClass.readDataFromExcelFile(25, 0));
		flag = false;
	}
